...
const SearchFeed = () => {
  ...
  useEffect(() => {
    fetchApi(`search?part=snippet,id&q=${searchInput}`).then((data) => setSearchVideo(data.items));
  }, [searchInput]);
  ...
  if (searchVideo.length === 0) {
    ...
    return (
      <div className="md:grid md:gap-4 md:grid-cols-3 md:grid-cols-3">
        <h1 className="my-4">
          Hasil Pencarian : <span className="font-bold">{searchInput}</span>
        </h1>
        {searchVideo.map((video, i) => {
